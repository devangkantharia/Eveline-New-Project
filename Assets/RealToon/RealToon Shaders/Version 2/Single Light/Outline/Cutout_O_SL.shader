...
Shader "RealToon/Version 2/Single Light/Outline/Cutout" {
    Properties {
        [Header((RealToon V2.0.1))]
    	[Header((Single Light x Outline))]
    	[Header((Cutout))]
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv0 = v.texcoord0;
                float4 node_810 = _Time + _TimeEditor;
                float node_5371_ang = node_810.g;
...
////// Emissive:
                ...
                float node_8411 = (attenuation*node_9545);
                float node_99 = (_HardEdgeFresnel_var*lerp(0.0,_HardEdgeFresnel_var,(lerp( node_7969, node_8411, _FresnelOnLight )*lerp( node_7969, (1.0 - node_8411), _FresnelOnShadow ))));
                float3 node_137 = ((node_99*_FresnelColor.rgb)*_FresnelIntensity); 
                float3 node_7802 = ( lerp(0.5,1.0,_TextureWashout) > 0.5 ? (1.0-(1.0-2.0*(lerp(0.5,1.0,_TextureWashout)-0.5))*(1.0-_Texture_var.rgb)) : (2.0*lerp(0.5,1.0,_TextureWashout)*_Texture_var.rgb) );
                float3 node_4607 = (node_7802*_Color.rgb);
                float3 node_9033 = ((float3(float2(i.screenPos.r,7.5),0.0)+normalDirection+float3(float2((-2.4),i.screenPos.g),0.0))*0.2);
                float4 _FReflection_var = tex2D(_FReflection,TRANSFORM_TEX(node_9033, _FReflection));
                ...
                float3 node_2918 = lerp(float3(node_6873,node_6873,node_6873),lerp(lerp(float3(node_7147,node_7147,node_7147),(node_7802*0.9),_MainTextureColorAO),float3(node_1502,node_1502,node_1502),lerp(_AOTexture_var.rgb,dot(_AOTexture_var.rgb,float3(0.3,0.59,0.11)),1.0)),lerp(0.0,lerp(1.0,10.0,_MainTextureColorAO),_AOIntensity));
                float3 node_4224 = (node_3766*UNITY_LIGHTMODEL_AMBIENT.rgb*lerp( 1.0, node_2918, _ShowAOonAmbientLight ));
                ...
                float3 emissive = lerp(saturate(min((lerp((0.0*node_137),((node_99*_FresnelColor.rgb)*_FresnelIntensity),_FresnelVisibleOnDarkAmbientLight)+lerp(node_4224,lerp(node_4224,(_SelfLitColor.rgb*node_3766*_SelfLitPower),_MaskSelfLit_var.rgb),lerp(0.0,1.0,_SelfLitIntensity))),node_5038)),dot(saturate(min((lerp((0.0*node_137),((node_99*_FresnelColor.rgb)*_FresnelIntensity),_FresnelVisibleOnDarkAmbientLight)+lerp(node_4224,lerp(node_4224,(_SelfLitColor.rgb*node_3766*_SelfLitPower),_MaskSelfLit_var.rgb),lerp(0.0,1.0,_SelfLitIntensity))),node_5038)),float3(0.3,0.59,0.11)),node_3161);
                float node_8311 = (lerp(1.0,attenuation,_ReceiveShadowsIntensity)*lerp(1.0,node_9545,_SelfShadowIntensity));
                float node_8466 = lerp(1.0,node_8311,lerp(0.0,lerp(1.0,2.6,_MainTextureColorShadow),_ShadowIntensity));
                float node_6054 = (1.0 - node_8466);
                float node_6126 = 0.0;
                float node_6334 = 0.0;
                float node_557 = 1.0;
                ...
                float2 node_9013 = (1.0 - (reflect(viewDirection,normalDirection).rg*0.5+0.5));
                float4 _GlossTexture_var = tex2D(_GlossTexture,TRANSFORM_TEX(node_9013, _GlossTexture));
                float3 node_7775 = lerp(_GlossTexture_var.rgb,dot(_GlossTexture_var.rgb,float3(0.3,0.59,0.11)),0.0);
                float node_3465 = 0.0;
                float node_1227 = lerp((lerp( smoothstep( 0.79, 0.9, (node_9744*3.0) ), node_9744, _SoftGloss )*1.0),lerp( node_7775, lerp(float3(node_3465,node_3465,node_3465),node_7775,node_8411), _ShadowMaskGlossTexture ),_GlossTextureIntensity);
                float node_8863 = 0.0;
                ...
                float node_2761 = 0.0;
                float3 finalColor = emissive + lerp(saturate(min(((((node_3766*(lerp(float3(node_8466,node_8466,node_8466),lerp(lerp((node_6054*_ShadowColor.rgb),lerp(float3(node_6126,node_6126,node_6126),saturate((1.0-((1.0-0.44)/node_7802))),node_6054),_MainTextureColorShadow),lerp(float3(node_6334,node_6334,node_6334),(lerp(lerp(float3(node_557,node_557,node_557),_ShadowPTexture_var.rgb,_ShadowPTexture_var.a),float3(node_3933,node_3933,node_3933),node_8311)*lerp(0.54,1.65,_MainTextureColorShadow)),_ShadowIntensity),_ShadowPTextureIntensity),0.65)*2.86))*lerp( lerp(node_2918,float3(node_1302,node_1302,node_1302),node_8311), node_2918, _ShowAOonLight ))+(lerp(float3(node_8890,node_8890,node_8890),lerp( lerp(float3(node_1227,node_1227,node_1227),(node_1227*_GlossColor.rgb),2.22), lerp(float3(node_8863,node_8863,node_8863),node_7802,node_1227), _MainTextureColorGloss ),_GlossMask_var.rgb)*lerp((node_8388_if_leA*node_2583)+(node_8388_if_leB*_GlossIntensity),_GlossIntensity,node_8388_if_leA*node_8388_if_leB))+lerp(node_137,float3(node_2761,node_2761,node_2761),_FresnelVisibleOnDarkAmbientLight))*_LightColor0.rgb),node_5038)),dot(saturate(min(((((node_3766*(lerp(float3(node_8466,node_8466,node_8466),lerp(lerp((node_6054*_ShadowColor.rgb),lerp(float3(node_6126,node_6126,node_6126),saturate((1.0-((1.0-0.44)/node_7802))),node_6054),_MainTextureColorShadow),lerp(float3(node_6334,node_6334,node_6334),(lerp(lerp(float3(node_557,node_557,node_557),_ShadowPTexture_var.rgb,_ShadowPTexture_var.a),float3(node_3933,node_3933,node_3933),node_8311)*lerp(0.54,1.65,_MainTextureColorShadow)),_ShadowIntensity),_ShadowPTextureIntensity),0.65)*2.86))*lerp( lerp(node_2918,float3(node_1302,node_1302,node_1302),node_8311), node_2918, _ShowAOonLight ))+(lerp(float3(node_8890,node_8890,node_8890),lerp( lerp(float3(node_1227,node_1227,node_1227),(node_1227*_GlossColor.rgb),2.22), lerp(float3(node_8863,node_8863,node_8863),node_7802,node_1227), _MainTextureColorGloss ),_GlossMask_var.rgb)*lerp((node_8388_if_leA*node_2583)+(node_8388_if_leB*_GlossIntensity),_GlossIntensity,node_8388_if_leA*node_8388_if_leB))+lerp(node_137,float3(node_2761,node_2761,node_2761),_FresnelVisibleOnDarkAmbientLight))*_LightColor0.rgb),node_5038)),float3(0.3,0.59,0.11)),node_3161);
