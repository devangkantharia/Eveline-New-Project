...
Shader "RealToon/Version 2/Single Light/No Outline/Default" {
    Properties {
        [Header((RealToon V2.0.1))]
    	[Header((Single Light x No Outline))]
    	[Header((Default))]
...
////// Emissive:
                ...
                float node_5147 = (attenuation*node_9545);
                float node_7879 = (_HardEdgeFresnel_var*lerp(0.0,_HardEdgeFresnel_var,(lerp( node_9853, node_5147, _FresnelOnLight )*lerp( node_9853, (1.0 - node_5147), _FresnelOnShadow ))));
                float3 node_137 = ((node_7879*_FresnelColor.rgb)*_FresnelIntensity); 
                float3 node_9747 = ( lerp(0.5,1.0,_TextureWashout) > 0.5 ? (1.0-(1.0-2.0*(lerp(0.5,1.0,_TextureWashout)-0.5))*(1.0-_Texture_var.rgb)) : (2.0*lerp(0.5,1.0,_TextureWashout)*_Texture_var.rgb) );
                float3 node_4607 = (node_9747*_Color.rgb);
                float3 node_3469 = ((float3(float2(i.screenPos.r,7.5),0.0)+normalDirection+float3(float2((-2.4),i.screenPos.g),0.0))*0.2);
                float4 _FReflection_var = tex2D(_FReflection,TRANSFORM_TEX(node_3469, _FReflection));
                ...
                float node_7732 = 1.0;
                float node_695 = 0.0;
                float3 node_1752 = lerp(float3(node_695,node_695,node_695),(node_9747*0.9),_MainTextureColorAO);
                float node_132 = 1.0;
                float4 _AOTexture_var = tex2D(_AOTexture,TRANSFORM_TEX(i.uv0, _AOTexture));
                float node_8824 = 0.0;
                float node_1531 = 1.0;
                float node_6008 = 10.0;
                float3 node_5158 = lerp(float3(node_7732,node_7732,node_7732),lerp(node_1752,float3(node_132,node_132,node_132),lerp(_AOTexture_var.rgb,dot(_AOTexture_var.rgb,float3(0.3,0.59,0.11)),1.0)),lerp(float3(node_8824,node_8824,node_8824),lerp(float3(node_1531,node_1531,node_1531),float3(node_6008,node_6008,node_6008),node_1752),_AOIntensity));
                float3 node_4224 = (node_3766*UNITY_LIGHTMODEL_AMBIENT.rgb*lerp( 1.0, node_5158, _ShowAOonAmbientLight ));
                ...
                float3 emissive = lerp(saturate(min((lerp((0.0*node_137),((node_7879*_FresnelColor.rgb)*_FresnelIntensity),_FresnelVisibleOnDarkAmbientLight)+lerp(node_4224,lerp(node_4224,(_SelfLitColor.rgb*node_3766*_SelfLitPower),_MaskSelfLit_var.rgb),lerp(0.0,1.0,_SelfLitIntensity))),node_5038)),dot(saturate(min((lerp((0.0*node_137),((node_7879*_FresnelColor.rgb)*_FresnelIntensity),_FresnelVisibleOnDarkAmbientLight)+lerp(node_4224,lerp(node_4224,(_SelfLitColor.rgb*node_3766*_SelfLitPower),_MaskSelfLit_var.rgb),lerp(0.0,1.0,_SelfLitIntensity))),node_5038)),float3(0.3,0.59,0.11)),node_3161);
                float node_2605 = (lerp(1.0,attenuation,_ReceiveShadowsIntensity)*lerp(1.0,node_9545,_SelfShadowIntensity));
                float node_5741 = lerp(1.0,node_2605,lerp(0.0,lerp(1.0,2.6,_MainTextureColorShadow),_ShadowIntensity));
                float node_6054 = (1.0 - node_5741); 
                float node_4914 = 0.0;
                float node_653 = 0.0;
                float node_557 = 1.0;
                ...
                float2 node_4732 = (1.0 - (reflect(viewDirection,normalDirection).rg*0.5+0.5));
                float4 _GlossTexture_var = tex2D(_GlossTexture,TRANSFORM_TEX(node_4732, _GlossTexture));
                float3 node_5819 = lerp(_GlossTexture_var.rgb,dot(_GlossTexture_var.rgb,float3(0.3,0.59,0.11)),0.0);
                float node_1802 = 0.0;
                float node_6270 = lerp((lerp( smoothstep( 0.79, 0.9, (node_9744*3.0) ), node_9744, _SoftGloss )*1.0),lerp( node_5819, lerp(float3(node_1802,node_1802,node_1802),node_5819,node_5147), _ShadowMaskGlossTexture ),_GlossTextureIntensity);
                float node_3639 = 0.0;
                ...
                float node_9574 = 0.0;
                float3 finalColor = emissive + lerp(saturate(min(((((node_3766*(lerp(float3(node_5741,node_5741,node_5741),lerp(lerp((node_6054*_ShadowColor.rgb),lerp(float3(node_4914,node_4914,node_4914),saturate((1.0-((1.0-0.44)/_Texture_var.rgb))),node_6054),_MainTextureColorShadow),lerp(float3(node_653,node_653,node_653),(lerp(lerp(float3(node_557,node_557,node_557),_ShadowPTexture_var.rgb,_ShadowPTexture_var.a),float3(node_3933,node_3933,node_3933),node_2605)*lerp(0.54,1.65,_MainTextureColorShadow)),_ShadowIntensity),_ShadowPTextureIntensity),0.65)*2.86))*lerp( lerp(node_5158,float3(node_7654,node_7654,node_7654),node_2605), node_5158, _ShowAOonLight ))+(lerp(float3(node_6011,node_6011,node_6011),lerp( lerp(float3(node_6270,node_6270,node_6270),(node_6270*_GlossColor.rgb),2.22), lerp(float3(node_3639,node_3639,node_3639),node_9747,node_6270), _MainTextureColorGloss ),_GlossMask_var.rgb)*lerp((node_8388_if_leA*node_2583)+(node_8388_if_leB*_GlossIntensity),_GlossIntensity,node_8388_if_leA*node_8388_if_leB))+lerp(node_137,float3(node_9574,node_9574,node_9574),_FresnelVisibleOnDarkAmbientLight))*_LightColor0.rgb),node_5038)),dot(saturate(min(((((node_3766*(lerp(float3(node_5741,node_5741,node_5741),lerp(lerp((node_6054*_ShadowColor.rgb),lerp(float3(node_4914,node_4914,node_4914),saturate((1.0-((1.0-0.44)/_Texture_var.rgb))),node_6054),_MainTextureColorShadow),lerp(float3(node_653,node_653,node_653),(lerp(lerp(float3(node_557,node_557,node_557),_ShadowPTexture_var.rgb,_ShadowPTexture_var.a),float3(node_3933,node_3933,node_3933),node_2605)*lerp(0.54,1.65,_MainTextureColorShadow)),_ShadowIntensity),_ShadowPTextureIntensity),0.65)*2.86))*lerp( lerp(node_5158,float3(node_7654,node_7654,node_7654),node_2605), node_5158, _ShowAOonLight ))+(lerp(float3(node_6011,node_6011,node_6011),lerp( lerp(float3(node_6270,node_6270,node_6270),(node_6270*_GlossColor.rgb),2.22), lerp(float3(node_3639,node_3639,node_3639),node_9747,node_6270), _MainTextureColorGloss ),_GlossMask_var.rgb)*lerp((node_8388_if_leA*node_2583)+(node_8388_if_leB*_GlossIntensity),_GlossIntensity,node_8388_if_leA*node_8388_if_leB))+lerp(node_137,float3(node_9574,node_9574,node_9574),_FresnelVisibleOnDarkAmbientLight))*_LightColor0.rgb),node_5038)),float3(0.3,0.59,0.11)),node_3161);
